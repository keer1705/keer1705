<p align="center">
<img src="https://capsule-render.vercel.app/api?type=waving&height=300&color=gradient&text=Hi%20,%20I'm%20Keerthana%20S"/>
</p>
<p align="center">
<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=28&duration=2500&pause=1000&center=true&vCenter=true&width=800&lines=AI+%26+ML+Engineer;Building+LLM+Applications;RAG+Developer;Full+Stack+Developer;Always+Learning+Something+New+🚀"/>
</p>

---

# 💻 Tech Stack

<p align="center">
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" height="48"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg" height="48"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/fastapi/fastapi-original.svg" height="48"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nodejs/nodejs-original.svg" height="48"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" height="48"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" height="48"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" height="48"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/tailwindcss/tailwindcss-original.svg" height="48"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg" height="48"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/sqlite/sqlite-original.svg" height="48"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/docker/docker-original.svg" height="48"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/kubernetes/kubernetes-plain.svg" height="48"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" height="48"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg" height="48"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vscode/vscode-original.svg" height="48"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/linux/linux-original.svg" height="48"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/windows8/windows8-original.svg" height="48"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/tensorflow/tensorflow-original.svg" height="48"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/pytorch/pytorch-original.svg" height="48"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/scikitlearn/scikitlearn-original.svg" height="48"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/figma/figma-original.svg" height="48"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postman/postman-original.svg" height="48"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/anaconda/anaconda-original.svg" height="48"/>
</p>



# 🚀 Featured Projects

<table>
<tr>

<td width="50%">

### 🤖 Agents Building Agents

> Multi-agent AI platform where intelligent agents **plan, build, validate, and deploy other AI agents** using NVIDIA NeMo, LangGraph, and custom workflows.

**⚡ Tech**
`Python` `NeMo` `LangGraph` `Ollama`

</td>

<td width="50%">

### 📄 OCR RAG Chatbot

> AI chatbot capable of understanding **100+ page PDFs** using OCR, LangChain, Weaviate, and conversational memory.

**⚡ Tech**
`LangChain` `OCR` `Weaviate`

</td>

</tr>

<tr>

<td width="50%">

### 🧠 LuminED

> AI-powered adaptive learning platform with **emotion-aware learning**, personalized roadmaps, and an interactive coding environment.

**⚡ Tech**
`React` `Django` `TensorFlow`

</td>

<td width="50%">

### 🌾 AgriAssist

> Multilingual AI farming assistant supporting **voice, image, and text queries** using Gemma and RAG.

**⚡ Tech**
`Gemma` `RAG` `Speech AI`

</td>

</tr>

<tr>

<td colspan="2">

### 💪 Strivo

> Stroke rehabilitation platform that transforms EMG sensor data into **AI-powered recovery insights** for remote patient monitoring.

**⚡ Tech**
`React` `Python` `LangGraph`

</td>

</tr>

</table>


---

# 📫 Connect With Me

💼 LinkedIn: https://linkedin.com/in/keerthana-s-988429305


